...
class Bond(object):
    ...
    def BondPrice(self, YieldToMaturity):
        ...
        fair_price = sum(
            [
                ...
                / (1 + YieldToMaturity / self.coupon_frequency)
                ** (self.coupon_frequency * t)
                for t in dt
            ]
        ) + self.par_value / (1 + YieldToMaturity / self.coupon_frequency) ** (
            self.coupon_frequency * self.time_to_maturity
        )
        return fair_price
    ...
    def YieldToMaturity(self, price, guess):
        ...
        ytm_func = (
            lambda r: -(
                -price + (1 - (pow(1 + r, -coupon_periods))) * (coupon_payments / r)
            )
            / (pow(1 + r, -coupon_periods))
            - self.par_value
        )
    ...
    def Duration(self):
        ...
        while i <= coupon_periods:
            ...
            dur = (self.par_value * self.coupon_rate * tn) / (
                pv
                * self.coupon_frequency
                * (1 + (ytm / self.coupon_frequency)) ** (self.coupon_frequency * tn)
            )
    ...
    def DurationByPriceSensitivity(self, yield_change):
        ...
        modified_duration = (price_minus - price_plus) / (
            2 * self.market_price * yield_change
        )
    ...
    def convexity(self, yield_change):
        ...
        ytm_plus = ytm + yield_change
        price_plus = self.BondPrice(ytm_plus)
        convexity = (price_minus + price_plus - 2 * self.market_price) / (
            self.market_price * yield_change**2
        )
        return convexity
