...
try:
    from ollama_analyzer import (
        OllamaConfig, LLMAnalysis,
        analyze_photo as llm_analyze,
        explain_rejection as llm_explain,
        categorize_photo as llm_categorize,
        run_custom_prompt as llm_run_custom,
        check_service as llm_check_service,
    )
    HAS_OLLAMA = True
except ImportError:
    HAS_OLLAMA = False
...
class Config:
    ...
    llm_max_image_size: int = 1024
    llm_explain_rejections: bool = True    # Generate human-readable rejection reasons
    llm_categorize: bool = True            # Auto-tag photos
    llm_aesthetic_scoring: bool = False     # Let LLM influence the final score
    llm_aesthetic_weight: float = 0.1      # Blend: (1-w)*heuristic + w*llm_opinion
    llm_custom_prompt: str = ""             # User's free-form prompt per photo

    def get_ollama_config(self) -> "OllamaConfig":
        """Build OllamaConfig from our settings."""
        if not HAS_OLLAMA:
            return None
        return OllamaConfig(
            ...
            max_image_size=self.llm_max_image_size,
            temperature=self.llm_temperature,
            custom_prompt=self.llm_custom_prompt,
        )
...
class PhotoResult:
    ...
    llm_quality_opinion: str = ""
    llm_suggested_profile: str = ""
    llm_custom_response: str = ""
...
def evaluate_photo(image_path: str, cfg: Config) -> PhotoResult:
    ...
    # ── 6. LLM enrichment (runs after heuristics) ─────────────────────────
    llm_desc = ""
    llm_critique = ""
    llm_caption = ""
    ...
    if cfg.llm_enabled and HAS_OLLAMA:
        ollama_cfg = cfg.get_ollama_config()
        if ollama_cfg and ollama_cfg.enabled:
            try:
                # Full structured analysis
                llm_result = llm_analyze(image_path, ollama_cfg)
                if llm_result.error is None:
                    llm_desc = llm_result.scene_description
                    llm_critique = llm_result.aesthetic_critique
                    llm_caption = llm_result.suggested_caption
                    llm_categories = llm_result.categories
                    llm_quality = llm_result.quality_opinion
                    llm_profile = llm_result.suggested_profile

                    # Optional: blend LLM opinion into score
                    if cfg.llm_aesthetic_scoring:
                        llm_score_map = {"good": 85, "mediocre": 50, "poor": 20}
                        llm_numeric = llm_score_map.get(llm_quality, total)
                        w = cfg.llm_aesthetic_weight
                        total = max(0, min(100, round(
                            (1 - w) * total + w * llm_numeric
                        )))
                        is_good = total >= cfg.min_score
                        notes.append(f"llm_aesthetic({llm_quality})\u2192blended")
                else:
                    notes.append(f"llm_error: {llm_result.error}")

                # Human-readable rejection explanation
                if not is_good and cfg.llm_explain_rejections:
                    llm_rejection = llm_explain(
                        image_path, breakdown, notes, ollama_cfg,
                    )

                # Auto-categorise (fallback if main analysis didn't return tags)
                if cfg.llm_categorize and not llm_categories:
                    llm_categories = llm_categorize(image_path, ollama_cfg)

                # Custom user prompt
                if cfg.llm_custom_prompt:
                    llm_custom = llm_run_custom(
                        image_path, cfg.llm_custom_prompt, ollama_cfg,
                    )
    ...
    return PhotoResult(
        ...
        llm_rejection_reason=llm_rejection,
        llm_quality_opinion=llm_quality,
        llm_suggested_profile=llm_profile,
        llm_custom_response=llm_custom,
    )
...
def generate_html_report(
    ...
    for r in sorted_results:
        ...
        # LLM enrichment blocks
        llm_html = ""
        has_llm = hasattr(r, 'llm_description') and (
            r.llm_description or r.llm_critique or r.llm_caption
        )
        if has_llm:
            llm_parts = []
            if getattr(r, 'llm_caption', ''):
                llm_parts.append(f'<div class="llm-caption">&#128221; {r.llm_caption}</div>')
            if getattr(r, 'llm_description', ''):
                llm_parts.append(f'<div class="llm-desc">&#128269; {r.llm_description}</div>')
            if getattr(r, 'llm_critique', ''):
                llm_parts.append(f'<div class="llm-critique">&#127912; {r.llm_critique}</div>')
            if getattr(r, 'llm_rejection_reason', ''):
                llm_parts.append(f'<div class="llm-reject">&#10060; {r.llm_rejection_reason}</div>')
            if getattr(r, 'llm_quality_opinion', ''):
                llm_parts.append(f'<div class="llm-quality">Quality: {r.llm_quality_opinion}</div>')
            if getattr(r, 'llm_suggested_profile', ''):
                llm_parts.append(f'<div class="llm-profile">Suggested profile: {r.llm_suggested_profile}</div>')
            if getattr(r, 'llm_custom_response', ''):
                llm_parts.append(f'<div class="llm-custom">&#128172; {r.llm_custom_response}</div>')
            cats = getattr(r, 'llm_categories', [])
            if cats:
                tags_html = "".join(
                    f'<span class="cat-tag">{c}</span>' for c in cats
                )
                llm_parts.append(f'<div class="llm-cats">{tags_html}</div>')
            llm_html = (
                '<details open><summary>&#129302; AI Analysis</summary>'
                '<div class="llm-block">' + "".join(llm_parts) + '</div>'
                '</details>'
            )

        llm_cats_str = ','.join(getattr(r, 'llm_categories', []))

        cards_html += f"""
        <div class="card" data-good="{1 if r.is_good else 0}"
             data-cats="{llm_cats_str}">
...
.llm-block{{margin:8px 0;padding:8px;background:#1a1a28;border-radius:6px;font-size:.85em}}
.llm-caption{{color:#f0c674;margin-bottom:4px;font-style:italic}}
.llm-desc{{color:#b0b0c0;margin-bottom:4px}}
.llm-critique{{color:#a0a0b8;margin-bottom:4px}}
.llm-reject{{color:#e23e57;margin-bottom:4px}}
.llm-quality{{color:#888;margin-bottom:4px;font-size:.8em}}
.llm-profile{{color:#888;margin-bottom:4px;font-size:.8em}}
.llm-custom{{color:#7F77DD;margin-bottom:4px;border-left:2px solid #7F77DD;padding-left:6px}}
.llm-cats{{margin-top:6px}}
.cat-tag{{display:inline-block;padding:2px 8px;margin:2px;border-radius:10px;
  background:#252540;color:#b0b0d0;font-size:.75em}}
.cat-bar{{text-align:center;margin-bottom:20px}}
.cat-bar button{{padding:4px 12px;margin:2px;border:none;border-radius:12px;
  cursor:pointer;background:#252540;color:#aaa;font-size:.8em}}
.cat-bar button.active{{background:#7F77DD;color:#fff}}
</style></head><body>
...
{_build_category_bar(sorted_results)}
...
function filterByCat(cat, btn) {{
  if (currentCat === cat) {{ currentCat = ''; btn.classList.remove('active'); }}
  else {{
    document.querySelectorAll('.cat-bar button').forEach(b => b.classList.remove('active'));
    currentCat = cat; btn.classList.add('active');
  }}
  applyFilters();
}}
function applyFilters() {{
  document.querySelectorAll('.card').forEach(c => {{
    ...
    let show = (currentFilter === 'all' ||
                (currentFilter === 'good' && isGood) ||
                (currentFilter === 'bad' && !isGood));
    if (show && currentCat) show = cats.split(',').includes(currentCat);
...
def _build_category_bar(results):
    """Build LLM category filter buttons for the HTML report."""
    all_cats = sorted({
        c for r in results
        for c in getattr(r, 'llm_categories', [])
    })
    if not all_cats:
        return ""
    buttons = "".join(
        f'<button onclick="filterByCat(\'{c}\',this)">{c}</button>'
        for c in all_cats
    )
    return f'<div class="cat-bar">Categories: {buttons}</div>'
    ...
    def _eval_or_cache(fpath: str) -> PhotoResult:
        ...
        if cache and fhash:
            cache.put(fhash, result.filename, {
                ...
                "llm_critique": result.llm_critique,
                "llm_caption": result.llm_caption,
                "llm_categories": result.llm_categories,
                "llm_rejection_reason": result.llm_rejection_reason,
                "llm_quality_opinion": result.llm_quality_opinion,
                "llm_suggested_profile": result.llm_suggested_profile,
                "llm_custom_response": result.llm_custom_response,
            })
        return result
            ...
            "photos": [
                {
                    ...
                    # LLM fields
                    "llm_description": getattr(r, 'llm_description', ''),
                    "llm_critique": getattr(r, 'llm_critique', ''),
                    "llm_caption": getattr(r, 'llm_caption', ''),
                    "llm_categories": getattr(r, 'llm_categories', []),
                    "llm_rejection_reason": getattr(r, 'llm_rejection_reason', ''),
                    "llm_quality_opinion": getattr(r, 'llm_quality_opinion', ''),
                    "llm_suggested_profile": getattr(r, 'llm_suggested_profile', ''),
                    "llm_custom_response": getattr(r, 'llm_custom_response', ''),
                }
...
def parse_args():
    ...
    p.add_argument("--llm-model",   default="llava:13b",
                   help="Ollama model name (default: llava:13b)")
    p.add_argument("--llm-url",     default="http://localhost:11434",
                   help="Ollama API base URL")
    p.add_argument("--llm-aesthetic", action="store_true",
                   help="Let LLM opinion influence the final score")
    p.add_argument("--llm-weight",  type=float, default=0.1,
                   help="LLM aesthetic blend weight (0.0-1.0)")
    p.add_argument("--no-llm-explain", action="store_true",
                   help="Skip LLM rejection explanations")
    p.add_argument("--no-llm-categorize", action="store_true",
                   help="Skip LLM photo categorization")
    p.add_argument("--llm-prompt",  default="",
                   help="Custom prompt to run against each photo")
    ...
    cfg = Config(
        ...
        llm_aesthetic_scoring=args.llm_aesthetic,
        llm_aesthetic_weight=args.llm_weight,
        llm_explain_rejections=not args.no_llm_explain,
        llm_categorize=not args.no_llm_categorize,
        llm_custom_prompt=args.llm_prompt,
    )
