...
class PhotoClassifierGUI:
    ...
    def _init_vars(self):
        ...
        # LLM / Ollama variables
        self.llm_enabled    = tk.BooleanVar(value=False)
        self.llm_model      = tk.StringVar(value="llava:13b")
        self.llm_url        = tk.StringVar(value="http://localhost:11434")
        self.llm_aesthetic   = tk.BooleanVar(value=False)
        self.llm_weight     = tk.DoubleVar(value=0.1)
        self.llm_explain    = tk.BooleanVar(value=True)
        self.llm_categorize = tk.BooleanVar(value=True)
        self.llm_prompt_text = ""   # stored as plain string, read from Text widget
        ...
        for v in [self.profile, self.input_dir, self.good_dir, self.bad_dir,
                  ...
                  self.llm_enabled, self.llm_model, self.llm_url,
                  self.llm_aesthetic, self.llm_weight,
                  self.llm_explain, self.llm_categorize,
                  *self.weights.values()]:
    ...
    def _build_left(self, parent):
        ...
        nb.add(self._tab_llm(nb),        text=" \U0001F916 LLM ")
    ...
    def _tab_llm(self, parent):
        tab = tk.Frame(parent, bg=BG2)
        inner = self._scroll_frame(tab)
        p = inner; p.configure(padx=16, pady=12)

        # ── Enable toggle ──
        section_label(p, "Ollama LLM Integration").pack(fill="x", pady=(0,6))
        self._toggle_row(p, self.llm_enabled,
                         "Enable LLM analysis",
                         "Requires Ollama running locally")

        # ── Status indicator ──
        status_row = tk.Frame(p, bg=BG2)
        status_row.pack(fill="x", pady=(4, 12))
        self.llm_status_lbl = label(status_row, "\u25cf Not checked", fg=TEXT2)
        self.llm_status_lbl.pack(side="left")
        tk.Button(status_row, text="Check Connection",
                  command=self._check_ollama_status,
                  bg=BG3, fg=TEXT2, font=FONT_TINY, relief="flat",
                  cursor="hand2", padx=8).pack(side="right")

        # ── Model selection ──
        section_label(p, "Model").pack(fill="x", pady=(0,6))
        model_row = tk.Frame(p, bg=BG2)
        model_row.pack(fill="x", pady=4)
        label(model_row, "Model name", fg=TEXT2, font=FONT_TINY).pack(anchor="w")
        r = tk.Frame(model_row, bg=BG2)
        r.pack(fill="x")
        styled_entry(r, self.llm_model).pack(side="left", fill="x", expand=True)
        label(p, "Recommended: llava:13b (vision) \u00b7 llava:7b (lighter) \u00b7 llama3.2-vision:11b",
              fg=TEXT2, font=FONT_TINY, wraplength=380).pack(anchor="w", pady=(2,8))

        # ── API URL ──
        section_label(p, "Connection").pack(fill="x", pady=(0,6))
        url_row = tk.Frame(p, bg=BG2)
        url_row.pack(fill="x", pady=4)
        label(url_row, "Ollama API URL", fg=TEXT2, font=FONT_TINY).pack(anchor="w")
        styled_entry(url_row, self.llm_url, width=36).pack(fill="x")

        # ── Feature toggles ──
        section_label(p, "LLM Features").pack(fill="x", pady=(10,6))
        self._toggle_row(p, self.llm_explain,
                         "Rejection explanations",
                         "Natural language reasons for rejected photos")
        self._toggle_row(p, self.llm_categorize,
                         "Auto-categorization",
                         "Tag photos with scene/event categories")
        self._toggle_row(p, self.llm_aesthetic,
                         "Aesthetic scoring (experimental)",
                         "Blend LLM quality opinion into final score")

        # ── Aesthetic weight slider ──
        self._scale_row(p, "LLM blend weight",
                        self.llm_weight, 0.0, 0.5, 0.05, "")
        label(p, "Only applies when aesthetic scoring is enabled. "
              "0.1 = 10% LLM, 90% heuristic.",
              fg=TEXT2, font=FONT_TINY, wraplength=380).pack(anchor="w", pady=(2,0))

        # ── Custom prompt text box ──
        section_label(p, "Custom Prompt").pack(fill="x", pady=(14,6))
        label(p, "This prompt is sent to the vision model for every photo. "
              "Leave empty to skip. The response appears in the report.",
              fg=TEXT2, font=FONT_TINY, wraplength=380).pack(anchor="w", pady=(0,6))

        prompt_frame = tk.Frame(p, bg=BG3, bd=0,
                                highlightthickness=1,
                                highlightbackground=BORDER)
        prompt_frame.pack(fill="x", pady=(0, 4))

        self.llm_prompt_widget = tk.Text(
            prompt_frame, height=5, bg=BG3, fg=TEXT,
            insertbackground=TEXT, relief="flat", wrap="word",
            font=FONT_MONO, undo=True,
            selectbackground=BG2, selectforeground=ACCENT,
        )
        self.llm_prompt_widget.pack(fill="x", padx=4, pady=4)
        self.llm_prompt_widget.insert("1.0",
            "Describe what makes this photo special or worth keeping. "
            "Mention any notable details about the subjects, setting, and mood.")

        # Bind keypress to refresh command preview
        self.llm_prompt_widget.bind("<KeyRelease>",
                                    lambda e: self._refresh_command())

        # ── Prompt examples ──
        examples_frame = tk.Frame(p, bg=BG2)
        examples_frame.pack(fill="x", pady=(4, 4))
        label(examples_frame, "Quick prompts:", fg=TEXT2,
              font=FONT_TINY).pack(anchor="w")

        prompt_examples = [
            ("Describe scene", "Describe this photo in one detailed paragraph."),
            ("Find issues", "List any technical issues with this photo: blur, noise, exposure, composition problems."),
            ("Caption", "Write a short, engaging social media caption for this photo."),
            ("Rate 1-10", "Rate this photo from 1 to 10 and explain your rating in one sentence."),
        ]
        btn_row = tk.Frame(p, bg=BG2)
        btn_row.pack(fill="x", pady=(2, 8))
        for btn_label, prompt_text in prompt_examples:
            def _set_prompt(t=prompt_text):
                self.llm_prompt_widget.delete("1.0", "end")
                self.llm_prompt_widget.insert("1.0", t)
                self._refresh_command()
            tk.Button(btn_row, text=btn_label, command=_set_prompt,
                      bg=BG3, fg=TEXT2, font=FONT_TINY, relief="flat",
                      cursor="hand2", padx=6, pady=2).pack(side="left", padx=2)
    ...
    def _check_ollama_status(self):
        ...
        def _check():
            try:
                import requests
                url = self.llm_url.get()
                resp = requests.get(f"{url}/api/tags", timeout=5)
                if resp.status_code == 200:
                    models = [m["name"] for m in resp.json().get("models", [])]
                    model_base = self.llm_model.get().split(":")[0]
                    found = any(model_base in m for m in models)
                    if found:
                        self.root.after(0, lambda: self.llm_status_lbl.configure(
                            text=f"\u25cf Connected \u2014 {self.llm_model.get()} available",
                            fg=GOOD))
                    else:
                        avail = ", ".join(models[:5]) or "none"
                        self.root.after(0, lambda avail=avail: self.llm_status_lbl.configure(
                            text=f"\u25cf Connected but model not found. Available: {avail}",
                            fg=WARN))
                else:
                    self.root.after(0, lambda: self.llm_status_lbl.configure(
                        text="\u25cf Ollama responded with error", fg=BAD))
            except Exception:
                self.root.after(0, lambda: self.llm_status_lbl.configure(
                    text=f"\u25cf Cannot reach Ollama at {self.llm_url.get()}",
                    fg=BAD))

        self.llm_status_lbl.configure(text="\u25cf Checking\u2026", fg=TEXT2)
    ...
    def _build_command(self):
        ...
        if self.llm_enabled.get():
            parts.append("--llm")
            if self.llm_model.get() != "llava:13b":
                parts += ["--llm-model", self.llm_model.get()]
            if self.llm_url.get() != "http://localhost:11434":
                parts += ["--llm-url", self.llm_url.get()]
            if self.llm_aesthetic.get():
                parts.append("--llm-aesthetic")
                parts += ["--llm-weight", str(self.llm_weight.get())]
            if not self.llm_explain.get():
                parts.append("--no-llm-explain")
            if not self.llm_categorize.get():
                parts.append("--no-llm-categorize")
            # Read prompt from Text widget
            prompt = self._get_llm_prompt()
            if prompt:
                parts += ["--llm-prompt", prompt]

        return parts

    def _get_llm_prompt(self):
        """Read the custom prompt from the LLM tab Text widget."""
        if hasattr(self, 'llm_prompt_widget'):
            text = self.llm_prompt_widget.get("1.0", "end").strip()
            return text
        return ""
